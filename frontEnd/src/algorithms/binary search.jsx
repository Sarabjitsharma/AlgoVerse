...
const BinarySearch = () => {
  ...
  const [explanation, setExplanation] = useState('Enter a sorted array and a target value to begin');
  ...
  const resetSearch = () => {
    ...
    setSteps([]);
    setShowExplanations([]);
    setExplanation('Enter a sorted array and a target value to begin');
    if (synthRef.current && synthRef.current.speaking) {
      synthRef.current.cancel();
    }
  };
